...
**v0.57.0 is yanked-effective for the Minija fixture.** Users who already pulled v0.57.0 should upgrade to v0.57.1 to recover the proper Minija geometry. The v0.57.0 tag remains in git history as a marker of the broken state.

### Fixed
- **Minija fixture restored** to the v0.56.20 state (18,366 cells across Minija mainstem + Babrungas/Salantas/Salpe/Veiviržė tributaries + Atmata branch + CuronianLagoon + BalticCoast). CRS unchanged from pre-v0.57.0 (already EPSG:3035).
- **`scripts/_generate_wgbast_physical_domains.RIVERS` no longer lists `example_minija_basin`.** The omission is documented inline so future maintainers understand the cost of re-adding it. The 4 actual WGBAST rivers (Tornionjoki/Simojoki/Byskeälven/Mörrumsån) remain in the list and continue to be generated by this pipeline.

The other 4 v0.57.0 fixes (Edit Model lasso/split race, WGBAST CRS, export latitude, merge warning, rename CSVs) are unchanged and remain in effect.
