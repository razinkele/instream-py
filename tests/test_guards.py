...
class TestCheckCapacity:
    def test_no_warning_below_threshold(self, caplog):
        from instream.utils.guards import check_capacity
        with caplog.at_level(logging.WARNING):
...
class TestCheckPositive:
    def test_passes_on_positive_values(self):
        from instream.utils.guards import check_positive
        arr = np.array([1.0, 2.0, 3.0])
...
class TestGetLogger:
    def test_get_logger_returns_logger(self):
        from instream.utils.logging import get_logger
    ...
    def test_logger_name_has_instream_prefix(self):
        ...
        assert logger.name == "instream.hydraulics"
