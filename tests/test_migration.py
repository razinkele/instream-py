...
class TestDownstreamMigration:
    def test_fish_migrates_when_fitness_high(self):
        ...
        result = should_migrate(
            migration_fit=0.8, best_habitat_fit=0.3, life_history=1)  # anad_juve
        assert result is True

    def test_fish_stays_when_habitat_better(self):
        from instream.modules.migration import should_migrate
        result = should_migrate(
            migration_fit=0.2, best_habitat_fit=0.5, life_history=1)
        assert result is False

    def test_resident_never_migrates(self):
        from instream.modules.migration import should_migrate
        result = should_migrate(
            migration_fit=0.99, best_habitat_fit=0.01, life_history=0)  # resident
        assert result is False

    def test_anad_adult_never_migrates_downstream(self):
        from instream.modules.migration import should_migrate
        result = should_migrate(
            migration_fit=0.99, best_habitat_fit=0.01, life_history=2)  # anad_adult
        assert result is False
...
class TestOutmigrantTracking:
    def test_bin_by_length_class(self):
        from instream.modules.migration import bin_outmigrant
