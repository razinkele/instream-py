...
class TestMovementRadius:
    ...
    def test_large_fish_has_larger_radius(self):
        ...
        r_small = movement_radius(length=5.0, move_radius_max=20000,
                                   move_radius_L1=7, move_radius_L9=20)
        r_large = movement_radius(length=15.0, move_radius_max=20000,
                                   move_radius_L1=7, move_radius_L9=20)
        assert r_large > r_small

    def test_radius_bounded_by_max(self):
        from instream.modules.behavior import movement_radius
        r = movement_radius(length=100.0, move_radius_max=20000,
                            move_radius_L1=7, move_radius_L9=20)
        assert r <= 20000

    def test_radius_positive(self):
        from instream.modules.behavior import movement_radius
        r = movement_radius(length=3.0, move_radius_max=20000,
                            move_radius_L1=7, move_radius_L9=20)
        assert r > 0
...
class TestCandidateMask:
    ...
    def simple_setup(self):
        ...
        ts.alive[0] = True; ts.cell_idx[0] = 0; ts.length[0] = 10.0
        ts.alive[1] = True; ts.cell_idx[1] = 2; ts.length[1] = 15.0
    ...
    def test_mask_shape(self, simple_setup):
        ...
        mask = build_candidate_mask(ts, space, move_radius_max=20000,
                                     move_radius_L1=7, move_radius_L9=20)
    ...
    def test_includes_neighbors(self, simple_setup):
        ...
        mask = build_candidate_mask(ts, space, move_radius_max=150,
                                     move_radius_L1=1, move_radius_L9=100)
...
class TestFitness:
    ...
    def test_wet_cell_higher_than_dry(self):
        ...
        f_dry = fitness_for(
            activity="drift", length=10.0, weight=10.0,
            depth=0.0, velocity=0.0, light=0.9, turbidity=2.0, temperature=15.0,
            drift_conc=3.2e-10, search_prod=8e-7, search_area=20000.0,
            available_drift=1000.0, available_search=1000.0,
            available_shelter=1000.0, shelter_speed_frac=0.3,
            superind_rep=1, prev_consumption=0.0, step_length=1.0,
    ...
    def test_hide_fitness_is_negative(self):
        from instream.modules.behavior import fitness_for
        f = fitness_for(
            activity="hide", length=10.0, weight=10.0,
            depth=50.0, velocity=30.0, light=50.0, turbidity=2.0, temperature=15.0,
            drift_conc=3.2e-10, search_prod=8e-7, search_area=20000.0,
            available_drift=1000.0, available_search=1000.0,
            available_shelter=1000.0, shelter_speed_frac=0.3,
    ...
    def test_returns_zero_when_velocity_exceeds_max(self):
        from instream.modules.behavior import fitness_for
        f = fitness_for(
            activity="search", length=5.0, weight=3.0,
            depth=50.0, velocity=200.0, light=50.0, turbidity=2.0, temperature=15.0,
            drift_conc=3.2e-10, search_prod=8e-7, search_area=20000.0,
            available_drift=1000.0, available_search=1000.0,
            available_shelter=1000.0, shelter_speed_frac=0.3,
            superind_rep=1, prev_consumption=0.0, step_length=1.0,
...
class TestDepletion:
    ...
    def test_drift_decreases(self):
        ...
        deplete_resources(fish_order, chosen_cells, chosen_activities,
                         intake_amounts, fish_lengths, superind_reps,
                         available_drift, available_search, available_shelter, available_hiding)
