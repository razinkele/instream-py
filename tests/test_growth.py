...
class TestSwimSpeed:
    ...
    def test_drift_speed_with_shelter(self):
        ...
        speed = drift_swim_speed(velocity=50.0, fish_length=5.0,
                                 available_shelter=100.0, shelter_speed_frac=0.3)
        # 5²=25, shelter=100 > 25 → use shelter: 50 * 0.3 = 15
        np.testing.assert_allclose(speed, 15.0)

    def test_drift_speed_without_shelter(self):
        from instream.modules.growth import drift_swim_speed
        speed = drift_swim_speed(velocity=50.0, fish_length=15.0,
                                 available_shelter=100.0, shelter_speed_frac=0.3)
...
class TestDriftIntake:
    ...
    def test_basic_drift_intake_positive(self):
        ...
        result = drift_intake(
            length=10.0, depth=50.0, velocity=30.0, light=50.0,
            turbidity=2.0, drift_conc=3.2e-10, max_swim_speed=50.0,
            c_stepmax_val=10.0, available_drift=1000.0, superind_rep=1,
    ...
    def test_limited_by_cstepmax(self):
        ...
        result = drift_intake(
            ...
            available_drift=1000.0, superind_rep=1,
            react_dist_A=4.0, react_dist_B=2.0,
            turbid_threshold=5.0, turbid_min=0.1, turbid_exp=-0.116,
            light_threshold=20.0, light_min=0.5, light_exp=-0.2,
            capture_R1=1.3, capture_R9=0.4,
    ...
    def test_zero_in_dry_cell(self):
        from instream.modules.growth import drift_intake
        result = drift_intake(
            length=10.0, depth=0.0, velocity=0.0, light=0.9,
            turbidity=2.0, drift_conc=3.2e-10, max_swim_speed=50.0,
            c_stepmax_val=10.0, available_drift=1000.0, superind_rep=1,
            react_dist_A=4.0, react_dist_B=2.0,
            turbid_threshold=5.0, turbid_min=0.1, turbid_exp=-0.116,
    ...
    def test_decreases_in_turbid_water(self):
        from instream.modules.growth import drift_intake
        kwargs = dict(length=10.0, depth=50.0, velocity=30.0, light=50.0,
                      drift_conc=3.2e-10, max_swim_speed=50.0,
                      c_stepmax_val=100.0, available_drift=1000.0, superind_rep=1,
                      react_dist_A=4.0, react_dist_B=2.0,
                      turbid_threshold=5.0, turbid_min=0.1, turbid_exp=-0.116,
                      light_threshold=20.0, light_min=0.5, light_exp=-0.2,
                      capture_R1=1.3, capture_R9=0.4)
    ...
    def test_decreases_in_low_light(self):
        from instream.modules.growth import drift_intake
        kwargs = dict(length=10.0, depth=50.0, velocity=30.0, turbidity=2.0,
                      drift_conc=3.2e-10, max_swim_speed=50.0,
                      c_stepmax_val=100.0, available_drift=1000.0, superind_rep=1,
                      react_dist_A=4.0, react_dist_B=2.0,
                      turbid_threshold=5.0, turbid_min=0.1, turbid_exp=-0.116,
    ...
    def test_limited_by_available_drift(self):
        ...
        result = drift_intake(
            length=10.0, depth=50.0, velocity=30.0, light=50.0,
            turbidity=2.0, drift_conc=3.2e-10, max_swim_speed=50.0,
            c_stepmax_val=100.0, available_drift=0.0001, superind_rep=1,
            react_dist_A=4.0, react_dist_B=2.0,
            turbid_threshold=5.0, turbid_min=0.1, turbid_exp=-0.116,
            light_threshold=20.0, light_min=0.5, light_exp=-0.2,
            capture_R1=1.3, capture_R9=0.4,
...
class TestSearchIntake:
    ...
    def test_positive_when_velocity_below_max(self):
        ...
        result = search_intake(
            velocity=20.0, max_swim_speed=50.0, search_prod=8e-7,
            search_area=20000.0, c_stepmax_val=100.0,
            available_search=1000.0, superind_rep=1,
        )
        assert result > 0.0
    ...
    def test_zero_when_velocity_exceeds_max(self):
        from instream.modules.growth import search_intake
        result = search_intake(
            velocity=60.0, max_swim_speed=50.0, search_prod=8e-7,
            search_area=20000.0, c_stepmax_val=100.0,
            available_search=1000.0, superind_rep=1,
        )
        assert result == 0.0
    ...
    def test_limited_by_cstepmax(self):
        from instream.modules.growth import search_intake
        result = search_intake(
            velocity=10.0, max_swim_speed=50.0, search_prod=8e-7,
            search_area=20000.0, c_stepmax_val=0.0001,
            available_search=1000.0, superind_rep=1,
        )
        assert result <= 0.0001
    ...
    def test_limited_by_available_search(self):
        ...
        result = search_intake(
            velocity=10.0, max_swim_speed=50.0, search_prod=8e-7,
            search_area=20000.0, c_stepmax_val=100.0,
            available_search=0.00001, superind_rep=1,
        )
...
class TestRespiration:
    ...
    def test_increases_with_swim_speed(self):
        from instream.modules.growth import respiration
        kwargs = dict(resp_std_wt_term=36 * 10**0.783,
                      resp_temp_term=np.exp(0.002 * 15**2),
                      max_swim_speed=50.0, resp_D=1.4)
    ...
    def test_overflow_guard(self):
        ...
        from instream.modules.growth import respiration
        result = respiration(
            resp_std_wt_term=100.0, resp_temp_term=1.0,
            swim_speed=1000.0, max_swim_speed=1.0,  # ratio=1000 >> 20
            resp_D=1.4,
        )
...
class TestGrowthRate:
    ...
    def test_growth_drift_is_intake_minus_respiration(self):
        ...
        result = growth_rate_for(
            activity="drift",
            length=10.0, weight=10.0, depth=50.0, velocity=30.0, light=50.0,
            turbidity=2.0, temperature=15.0,
...
class TestGrow:
    ...
    def test_positive_growth_increases_weight(self):
        ...
        new_w, new_l, new_k = apply_growth(
            weight=10.0, length=10.0, condition=1.0, growth=0.5,
    ...
    def test_weight_never_negative(self):
        ...
        new_w, _, _ = apply_growth(
            weight=0.1, length=5.0, condition=0.5, growth=-10.0,
            weight_A=0.0041, weight_B=3.49)
        assert new_w >= 0.0
    ...
    def test_condition_drops_with_weight_loss(self):
        ...
        _, _, new_k = apply_growth(
            weight=10.0, length=10.0, condition=1.0, growth=-2.0,
            weight_A=0.0041, weight_B=3.49)
        assert new_k < 1.0
    ...
    def test_length_increases_when_weight_exceeds_healthy(self):
        ...
        new_w, new_l, new_k = apply_growth(
            weight=healthy_w, length=10.0, condition=1.0, growth=5.0,
            weight_A=0.0041, weight_B=3.49)
        assert new_l > 10.0
    ...
    def test_length_never_decreases(self):
        ...
        _, new_l, _ = apply_growth(
            weight=10.0, length=10.0, condition=1.0, growth=-5.0,
            weight_A=0.0041, weight_B=3.49)
        assert new_l >= 10.0
